...
class QwenAIAdapter extends IAIService {
  ...
  _createResilientTimer(fn, interval, timerName = 'unnamed') {
    ...
    const wrappedFn = async () => {
      try {
        ...
        Logger.warn('QwenAIAdapter', 'TimerExecutionFailed', {
          timerName: timerName,
          consecutiveFailures: consecutiveFailures,
          errorType: error.name || 'TimerError',
          errorMsg: error.message || 'Timer execution failed',
          errorCode: 'ERR_QWEN_TIMER_EXEC',
        ...
        if (consecutiveFailures >= maxConsecutiveFailures) {
          ...
          Logger.error('QwenAIAdapter', 'TimerStoppedDueToFailures', {
            timerName: timerName,
            consecutiveFailures: consecutiveFailures,
            errorType: error.name || 'TimerError',
            errorMsg: `Timer stopped after ${maxConsecutiveFailures} failures`,
            errorCode: 'ERR_QWEN_TIMER_STOPPED',
            fallback: 'stop_timer',
        ...
        setTimeout(() => {
          try {
            ...
            Logger.error('QwenAIAdapter', 'TimerRetryFailed', {
              timerName: timerName,
              errorType: retryError.name || 'TimerError',
              errorMsg: retryError.message || 'Timer retry failed',
              errorCode: 'ERR_QWEN_TIMER_RETRY',
  ...
  _smartCleanupSessions() {
    try {
      ...
      Logger.warn('QwenAIAdapter', 'SmartCleanupFailed', {
        errorType: error.name || 'CleanupError',
        errorMsg: error.message || 'Smart cleanup sessions failed',
        errorCode: 'ERR_QWEN_CLEANUP',
        fallback: 'skip_operation',
        impact: 'no_impact'
      })
    }
  ...
  async generateHint(question, context = {}) {
    try {
      ...
      Logger.error('QwenAIAdapter', 'GenerateHintFailed', {
        question: (question || '').toString().substring(0, 100),
        context: context.context,
        errorType: error.name || 'AIError',
        errorMsg: error.message || 'Generate hint failed',
        errorCode: 'ERR_QWEN_GENERATE_HINT',
        fallback: 'return_fallback_response',
        impact: 'feature_degradation'
      })
  ...
  async analyzeLearningData(learningData) {
    try {
      ...
      Logger.error('QwenAIAdapter', 'AnalyzeLearningDataFailed', {
        errorType: error.name || 'AIError',
        errorMsg: error.message || 'Analyze learning data failed',
        errorCode: 'ERR_QWEN_ANALYZE_DATA',
        fallback: 'return_fallback_response',
        impact: 'feature_degradation'
      })
      return {
  ...
  async generateLearningPlan(userProfile, learningGoals) {
    try {
      ...
      Logger.error('QwenAIAdapter', 'GenerateLearningPlanFailed', {
        errorType: error.name || 'AIError',
        errorMsg: error.message || 'Generate learning plan failed',
        errorCode: 'ERR_QWEN_GENERATE_PLAN',
        fallback: 'return_fallback_response',
        impact: 'feature_degradation'
      })
...
4. 注意事项和建议`
  ...
  async _callCustomModel(prompt, options = {}) {
    try {
      ...
      Logger.error('QwenAIAdapter', 'CallCustomModelFailed', {
        modelName: this.customModelName,
        modelURL: this.customModelURL,
        errorType: error.name || 'AIError',
        errorMsg: error.message || 'Call custom model failed',
        errorCode: 'ERR_QWEN_CUSTOM_MODEL',
        fallback: 'fallback_to_official_api',
        impact: 'feature_degradation'
      })
      // 如果微调模型失败，尝试降级到官方API
      if (this.apiKey) {
  ...
  async sendMessage(userId, message) {
    ...
    try {
      ...
      Logger.error('QwenAIAdapter', 'SendMessageFailed', {
        userId: userId,
        messageLength: (message || '').length,
        errorType: error.name || 'AIError',
        errorMsg: error.message || 'Send message failed',
        errorCode: 'ERR_QWEN_SEND_MESSAGE',
...
4. 鼓励学生思考`
      ...
      Logger.error('QwenAIAdapter', 'GetHintFailed', {
        questionId: questionId,
        errorType: error.name || 'AIError',
        errorMsg: error.message || 'Get hint failed',
        errorCode: 'ERR_QWEN_GET_HINT',
        fallback: 'return_fallback_hint',
        impact: 'feature_degradation'
      })
      return {
      ...
      prompt += `\n\n请按照以下格式提供批改结果（JSON格式）:
{
  ...
  "detailedComments": "详细评语（100-200字）"
      ...
      Logger.error('QwenAIAdapter', 'GradeEssayFailed', {
        questionId: questionId,
        essayLength: essayContent?.length || 0,
        errorType: error.name || 'AIError',
        errorMsg: error.message || 'Grade essay failed',
        errorCode: 'ERR_QWEN_GRADE_ESSAY',
        fallback: 'return_fallback_grading',
        impact: 'feature_degradation'
      })
