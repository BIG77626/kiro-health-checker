...
class StorageAdapter {
  ...
  save(key, value) {
    try {
        ...
        try {
          ...
          if (e.message && e.message.includes('quota')) {
            ...
            try {
              ...
              Logger.error('StorageAdapter', 'SaveRetryFailed', {
                key: key,
                errorType: retryError.name || 'StorageError',
                errorMsg: retryError.message || 'Save retry failed',
                errorCode: 'ERR_STORAGE_SAVE_RETRY',
          ...
          Logger.error('StorageAdapter', 'WxSetStorageFailed', {
            key: key,
            errorType: e.name || 'StorageError',
            errorMsg: e.message || 'wx.setStorageSync failed',
            errorCode: 'ERR_STORAGE_WX_SET',
            fallback: 'return_false',
            impact: 'data_loss'
          })
          return false;
      ...
      Logger.error('StorageAdapter', 'SaveUnexpectedFailed', {
        key: key,
        errorType: e.name || 'UnexpectedError',
        errorMsg: e.message || 'Save failed unexpectedly',
        errorCode: 'ERR_STORAGE_SAVE_UNEXPECTED',
        fallback: 'return_false',
        impact: 'data_loss'
      })
      return false;
    ...
    try {
      ...
      if (this._platform === 'wechat') {
        try {
          ...
          Logger.error('StorageAdapter', 'WxGetStorageFailed', {
            key: key,
            errorType: e.name || 'StorageError',
            errorMsg: e.message || 'wx.getStorageSync failed',
            errorCode: 'ERR_STORAGE_WX_GET',
      ...
      Logger.error('StorageAdapter', 'LoadUnexpectedFailed', {
        key: key,
        errorType: e.name || 'UnexpectedError',
        errorMsg: e.message || 'Load failed unexpectedly',
        errorCode: 'ERR_STORAGE_LOAD_UNEXPECTED',
        fallback: 'return_null',
        impact: 'feature_degradation'
      })
      return null;
  ...
  remove(key) {
    try {
      ...
      Logger.error('StorageAdapter', 'RemoveUnexpectedFailed', {
        key: key,
        errorType: e.name || 'UnexpectedError',
        errorMsg: e.message || 'Remove failed unexpectedly',
        errorCode: 'ERR_STORAGE_REMOVE_UNEXPECTED',
        fallback: 'return_false',
        impact: 'no_impact'
      })
      return false;
  ...
  count(key) {
    try {
      ...
      Logger.error('StorageAdapter', 'CountFailed', {
        key: key,
        errorType: e.name || 'UnexpectedError',
        errorMsg: e.message || 'Count failed unexpectedly',
        errorCode: 'ERR_STORAGE_COUNT',
        fallback: 'return_zero',
        impact: 'no_impact'
      })
      return 0;
    }
  ...
  query(key, filter = {}) {
    try {
      ...
      Logger.error('StorageAdapter', 'QueryFailed', {
        key: key,
        errorType: e.name || 'UnexpectedError',
        errorMsg: e.message || 'Query failed unexpectedly',
        errorCode: 'ERR_STORAGE_QUERY',
        fallback: 'return_empty_array',
        impact: 'no_impact'
      })
      return [];
    }
  ...
  _clearOldData() {
    try {
      ...
      Logger.warn('StorageAdapter', 'ClearOldDataFailed', {
        errorType: e.name || 'CleanupError',
        errorMsg: e.message || 'Clear old data failed',
        errorCode: 'ERR_STORAGE_CLEANUP',
