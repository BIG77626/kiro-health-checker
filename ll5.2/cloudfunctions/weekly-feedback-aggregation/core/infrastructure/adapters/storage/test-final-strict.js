/**
 * 严格真机调试ERR_INVALID_ARG_TYPE错误测试脚本 - 最终版
 * 
 * 此脚本专门检查所有可能导致ERR_INVALID_ARG_TYPE错误的模式
 */
...
global.wx = {
  setStorage: function(options) {
    ...
    if (typeof options !== 'object' || !options.key) {
      if (options.fail) {
        // 在真机环境中，即使setTimeout也可能导致ERR_INVALID_ARG_TYPE错误
        // 这里模拟这种问题
        try {
          setTimeout(() => {
...
// 测试函数
async function runFinalStrictTest() {
  console.log('=== 最终严格真机调试ERR_INVALID_ARG_TYPE错误测试 ===')
...
// 运行测试
runFinalStrictTest().catch(error => {
